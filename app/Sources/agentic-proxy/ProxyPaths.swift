...
// 路径与 CLI 侧写死的一致: 配置目录 ~/.config/agentic-proxy (XDG_CONFIG_HOME 优先)。
...
nonisolated enum ProxyPaths {
    static var configDir: URL {
        ...
        return base.appendingPathComponent("agentic-proxy", isDirectory: true)
    }
    ...
    static func cli() -> URL? {
        ...
        let sibling = Bundle.main.executableURL?
            ...
            .appendingPathComponent("agentic-proxy-cli").path
        let candidates = [
            sibling,
            "\(home)/.local/bin/agentic-proxy",
            "/opt/homebrew/bin/agentic-proxy",
            "/usr/local/bin/agentic-proxy",
        ].compactMap { $0 }
        return candidates.first { FileManager.default.isExecutableFile(atPath: $0) }
            .map { URL(fileURLWithPath: $0) }
    }

    static let cliHint = "找不到 agentic-proxy: 先构建安装 app (CLI 在 app 包体内, 见工程 workflow.md)"
}
