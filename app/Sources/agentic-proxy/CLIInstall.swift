...
// 终端命令入口 ~/.local/bin/agentic-proxy -> 本 bundle 内的 CLI (symlink, 不拷贝)。
// app 是唯一副本: 换 app 即换 CLI, 入口自动跟随, 不存在两份二进制版本错位。
// 打开 app 时检查一次, 没装 / 指错就弹窗装 -> 用户全程只点一下, 不需要跑任何安装脚本。
nonisolated enum CLIInstall {
    static let linkName = "agentic-proxy"
    static let cliName = "agentic-proxy-cli"
