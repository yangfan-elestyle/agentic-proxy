...
- `app/package.sh [--arch arm64|x86_64]`: CLI + viewer 单架构构建 -> 组装 bundle -> ad-hoc 签名 -> `app/build/agentic-proxy.app`; 只构建, 不装机
...
- CLI 往返记录: `agentic-proxy logs -n 20` 看摘要; 原始行 `jq` 直接读 `~/.config/agentic-proxy/log/<日期>.jsonl`
- app 快编: `cd app && swift build` -> `./.build/debug/agentic-proxy`
- app 界面自检 (不需录屏授权): `./.build/debug/agentic-proxy --snapshot /tmp/app.png` -> 离屏渲染主窗口, 直接看 PNG
- 磁盘吃紧时 `./scripts/clean.sh` (先 `--dry-run` 看体积)
- 改完代理行为后先 `cargo build --release` + `./target/release/agentic-proxy start` 再打真实请求验证: daemon 跟着这份二进制跑, 不碰 `/Applications`; 与已装版共用 pid 文件 + 端口, start 自带 restart -> 不会双实例
...
- app: `cargo build --release` + `swift build -c release` -> 组装 bundle (viewer + `Contents/MacOS/agentic-proxy-cli`) -> 由内向外 ad-hoc 签名 (先 CLI 后 bundle, DR 钉 identifier-only) -> `pkill` 旧 viewer -> `ditto` 到 `/Applications`
- 终端入口: `ln -sfn` 把 `~/.local/bin/agentic-proxy` 指到包体内那份 -> `--version` 自检
- 只覆盖同名 app / 同名入口, 不影响 `~/.local/bin` 里其他 CLI
- 验证: 输出的版本与本次 tag 一致
...
> 旧进程仍在跑时: 装完执行 `agentic-proxy start` 即切到新版本 (start 自带 restart)。
...
> 脚本认不出仓库根 (`Cargo.toml` 里没有 `name = "agentic-proxy"`) 直接 exit 2, 不接受路径参数 —— 里面全是 `rm -rf`。
