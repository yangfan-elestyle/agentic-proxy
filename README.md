...
系统要求 macOS 13 及以上。在 [Releases](https://github.com/yangfan-elestyle/agentic-proxy/releases/latest) 页下载与本机架构对应的 `.dmg`：

<!-- prettier-ignore -->
| 机型 | 安装包 |
| --- | --- |
| Apple 芯片（M1 及之后） | `agentic-proxy-macos-arm64.dmg` |
| Intel | `agentic-proxy-macos-x86_64.dmg` |
...
记录按天存放于 `~/.config/agentic-proxy/log/`，不会自动清理。
...
```bash
agentic-proxy                # 启动（已在运行则重启）
agentic-proxy stop           # 停止
agentic-proxy status         # 运行状态、账号、套餐、局域网地址、固定 api key
agentic-proxy models         # 当前可用的 model
agentic-proxy logs -n 20     # 最近 20 条往返摘要
agentic-proxy login codex    # 或 login anthropic
agentic-proxy logout all     # 或 logout codex / logout anthropic
```
