...
async fn stop() -> Response {
    ...
    Json(json!({
        "ok": true,
        "message": "已发送停止信号; 重新启动需在终端执行 `agentic-proxy start`",
    }))
    .into_response()
}
...
mod tests {
    ...
    fn scan_resumes_and_leaves_partial_line() {
        let dir = std::env::temp_dir().join(format!("agentic-webui-scan-{}", std::process::id()));
    ...
    fn read_line_survives_truncated_tail() {
        let dir = std::env::temp_dir().join(format!("agentic-webui-line-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("2026-08-04.jsonl");
